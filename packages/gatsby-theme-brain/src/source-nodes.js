...
module.exports = ({ actions, createNodeId, createContentDigest }, pluginOptions) => {
  ...
  for (var slug in slugToNoteMap) {
    ...
    if (replacementMatches != null) {
      replacementMatches = replacementMatches.filter((a, b) => replacementMatches.indexOf(a) === b);
    ...
    if (note.outboundReferences) {
      // change them to be slugs instead of exact matches
      note.outboundReferences = note.outboundReferences.map(
        (ob) => nameToSlugMap[ob.toLowerCase()] || ob
      );
    }
    ...
    if (inboundReferences != null) {
      inboundReferences = inboundReferences.filter((a, b) => inboundReferences.indexOf(a) === b);
    }
